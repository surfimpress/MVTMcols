...
↳ **C1a landed — straightened the open-variant.** Switched `detect_headlines.py`, `detect_body_text.py`, and `validate_columns.py` from raw `fitz.open` to `pdf_utils.open_clean_pdf` (imported as `_open_clean`, matching the convention from B1). Removed the now-dead `import fitz` from each. Verified by importing all three modules and re-running the four-issue regression (1898-10-07, 1920-01-02, 1937-01-14, 1947-11-06): zero deltas across all five tables. **Conclusion: the bypass was drift, not design.** None of the regression issues had red-overlay annotations strong enough to cause a delta, but the principle is now consistent — every detector strips the overlay before measuring. DB backup at `data/mvtm.db.pre-C1a.bak`. **What's next (C1b):** with "clean vs raw" eliminated, only the dtype dimension remains. Add `render_grey()` (float64) and `render_grey_uint8()` (for `cv2.adaptiveThreshold`) to `pdf_utils.py`; consolidate the 9 detection-relevant render-to-grey callsites.
